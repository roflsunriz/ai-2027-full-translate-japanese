...
このリポジトリは `ai-2027.html` を中心にした静的な翻訳資料で構成されているため、特別なビルド手順なしに GitHub Pages で公開できます。以下の手順で誰でもブラウザで閲覧できるようになります。

## 前提

- このリポジトリを GitHub に push してあること
- 公開に利用するブランチが `main` など既存のブランチであること
- `ai-2027.html` と `style.css`、`favicon.png` など静的ファイルがルートにあること（既に満たされています）
...
5. 表示された URL をブラウザで開くと、`ai-2027.html` が自動的に表示されます。必要なら、`ai-2027.html` を直接参照（例: `https://<ユーザー名>.github.io/ai-2027/ai-2027.html`）して実装内容を確認します。
...
- PDF 版（`ai-2027.pdf`）も同じブランチにあるため、GitHub Pages 上からリンクを辿ることができます。必要なら `README.md` にリンクを加えておくと公開先の紹介に役立ちます。
